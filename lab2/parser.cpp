...
void Parser::fill_generated(TreeNode* parent, int tab, string test_folder) {
    ...
    string token_str = " ";
    if(parent->token != -1){
        token_str = to_string(parent->token);
    }
    out << parent->rule << " " << token_str << " " << parent->name << endl;
    cout << parent->rule << " " << token_str << " " << parent->name << endl;
    int i = 0;
...
void Parser::add_to_tree(TreeNode* parent, TreeNode* node){
    ...
    node->token = all_lexemes[0].get_code();
    node->name = all_lexemes[0].get_name();
    node->pos = "[col " + to_string(all_lexemes[0].get_column()) + "] [row " + to_string(all_lexemes[0].get_row()) + "]";
...
void Parser::attribute(TreeNode* parent) {
    ...
    node->rule = "<attribute>";
    syn_tree.insert(parent, node);
    if ((!all_lexemes.empty()) && (all_lexemes[0].get_code() > 404) && (all_lexemes[0].get_code() < 412)) {
        TreeNode *new_node = new TreeNode();
        add_to_tree(node, new_node);
    } else if(errors.empty()){
