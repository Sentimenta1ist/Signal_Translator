        ...
        while (!file.eof()) {
                ...
                while (!file.eof()) {
                    ...
                    if (curr_pair.second != number) {
                        if (curr_pair.second == letter) {
                            out << "Error: letter after digit.Col:" << curr_column << "Row:" << curr_row << endl;
                            printf("Error on line %d column %d. Letter after digit!\n", curr_row, curr_column);
                        } else {
                            bool flag = true;
                            int lex_code;
            ...
            if (curr_pair.second == token){
                ...
                while (!file.eof()){
                    ...
                    if((curr_pair.second == number) || (curr_pair.second == letter) || (curr_pair.second == whitespace)){
                        tmp_string.push_back(s);
                        next_smb(file, s, curr_column);
                        curr_pair = get_category(s);
                        while(!file.eof()){
            ...
            if ((curr_pair.second == unacceptable) && (!file.eof())) {
                out << "Error: Unaccepted token.Col:" << curr_column << "Row:" << curr_row << " - "  << s << endl;
                printf("Unaccepted token on column %d row %d |token: '%c'\n", curr_column, curr_row, s);
                while (!file.eof()) {
                    next_smb(file, s, curr_column);
                    curr_pair = get_category(s);
